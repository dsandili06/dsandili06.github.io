    ...
    <nav className="sticky top-0 z-50 bg-[color-mix(in_oklab,var(--background)_88%,transparent)] backdrop-blur-[12px] border-b border-border-dim">
      <div className="max-w-7xl mx-auto px-6 md:px-10 h-14 sm:h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-9 font-mono text-[11px] uppercase tracking-[0.18em]">
          {links.map((l) => {
            ...
            return (
              <a key={l.href} href={l.href} className={`relative pb-1 transition-colors duration-200 ${isActive ? "text-[var(--accent)]" : "text-foreground/75 hover:text-[var(--accent)]"}`}>
                {l.label}
                {isActive && (
                  <motion.span layoutId="nav-active-underline" className="absolute left-0 right-0 -bottom-0.5 h-px bg-[var(--accent)]" transition={{ type: "spring", stiffness: 380, damping: 30 }} />
                )}
              </a>
            );
          })}
        </div>
        <button type="button" onClick={() => setMenuOpen((v) => !v)} aria-label={menuOpen ? "Cerrar menú" : "Abrir menú"} aria-expanded={menuOpen} className="md:hidden inline-flex items-center justify-center w-10 h-10 -mr-2 text-[var(--accent)] border border-border-dim hover:border-[var(--accent)] transition-colors">
      ...
      <div className={`md:hidden overflow-hidden border-t border-border-dim bg-background/95 backdrop-blur-md transition-[max-height,opacity] duration-300 ${menuOpen ? "max-h-[80vh] opacity-100" : "max-h-0 opacity-0"}`}>
        <ul className="flex flex-col px-6 py-2 font-mono text-xs uppercase tracking-[0.2em]">
          {links.map((l) => (
            <li key={l.href}>
              <a href={l.href} onClick={() => setMenuOpen(false)} className="flex items-center gap-3 py-3 border-b border-border-dim/60 text-foreground/80 hover:text-[var(--accent)] transition-colors">
  ...
  return (
    <section id="top" className="relative min-h-[100dvh] flex flex-col justify-between overflow-hidden border-b border-border-dim grid-bg">
      <div className="absolute inset-0 -z-10 pointer-events-none" aria-hidden>
        <ParticlesBg count={90} connectionDistance={130} particleColor="59,130,246" lineColor="59,130,246" maxOpacity={0.25} maxLineOpacity={0.08} speed={0.25} />
      </div>
      ...
      <div className="max-w-7xl w-full mx-auto px-6 md:px-10 pb-10 flex items-end justify-between font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--muted-foreground)] gap-6 flex-wrap">
        <div className="flex items-center gap-6"><span>SCROLL ↓</span></div>
        <div className="text-right">
          <div>STATUS: <span className="text-[var(--accent-green)]">DISPONIBLE</span></div>
...
function Section({ id, number, title, kicker, children }: { id: string; number: string; title: string; kicker?: string; children: React.ReactNode }) {
  return (
...
function SectionHeader({ number, title, kicker }: { number: string; title: string; kicker?: string }) {
  return (
    <div className="relative mb-14 md:mb-20">
      <span aria-hidden className="absolute -top-6 md:-top-10 -left-2 md:-left-6 font-display font-bold leading-none text-[var(--surface)] select-none pointer-events-none" style={{ fontSize: "clamp(5rem, 14vw, 12rem)" }}>{number}</span>
      <div className="relative flex items-end justify-between gap-6 pb-5 border-b border-border-dim">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-[var(--muted-foreground)] mb-3">SECTION_{number}{kicker ? ` // ${kicker}` : ""}</div>
          <h2 className="font-display font-bold leading-none tracking-[-0.02em] text-[var(--accent)]" style={{ fontSize: "clamp(2rem, 5vw, 4rem)" }}>{title}</h2>
        </div>
    ...
    <Section id="about" number="01" title="About Me" kicker="PROFILE">
      <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-12 lg:gap-16 items-start">
        <div className="space-y-6 text-[15px] md:text-base leading-relaxed text-foreground/80 max-w-[58ch]">
          <p>Soy Santiago Daniel Sandili, analista SOC Jr. orientado a <span className="text-[var(--accent)]">Blue Team</span> y DFIR. Me fui formando de manera práctica con laboratorios, writeups y herramientas reales para desarrollar una base sólida en detección, triage y respuesta a incidentes.</p>
          <p>Vengo del interior del país (Argentina) y estoy construyendo mi camino en ciberseguridad desde un enfoque práctico. Mi interés principal está en entender cómo se detecta, investiga y contiene una amenaza a partir de evidencia real.</p>
          <p>Me gusta trabajar sobre memoria, artefactos de Windows, tráfico de red, logs y casos de malware para reconstruir lo que pasó y sacar conclusiones útiles. Por eso mi foco hoy está en <span className="text-[var(--accent)]">Blue Team</span>.</p>
        </div>
        <a href="https://assets.tryhackme.com/certification-certificate/69bb156d56eed3cbe3a712a6.pdf" target="_blank" rel="noreferrer" className="group block bg-[var(--surface)] border border-border-dim p-7 relative transition-colors hover:bg-[color-mix(in_oklab,var(--surface)_70%,var(--background))]" style={{ borderLeft: "3px solid var(--accent-green)" }}>
          <div className="flex items-center justify-between mb-6">
            <Badge variant="success" dot>CERTIFIED</Badge>
            <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--muted-foreground)]">2026</span>
          </div>
          <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--muted-foreground)] mb-3">TRYHACKME · SAL1</div>
          <h3 className="font-display font-bold leading-tight text-[1.75rem] md:text-[2rem] text-foreground mb-6 group-hover:text-[var(--accent)] transition-colors">Security Analyst L1</h3>
          <div className="border-t border-border-dim pt-5 flex items-end justify-between">
            <div>
              ...
              <div className="font-display font-bold text-3xl text-[var(--accent)] leading-none">948 <span className="text-[var(--muted-foreground)] text-xl">/ 1000</span></div>
            </div>
            <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--accent)] opacity-0 group-hover:opacity-100 transition-opacity">VER →</span>
          </div>
        </a>
      </div>
    </Section>
...
function Proyectos() {
  return (
    <Section id="proyectos" number="02" title="Proyectos" kicker="REPOSITORIES">
      <div className="flex flex-col">
        {PROJECTS.map((p, idx) => (
          <motion.a key={p.id} href={p.href} target="_blank" rel="noreferrer" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.55, delay: idx * 0.08, ease: [0.16, 1, 0.3, 1] }} className="group grid grid-cols-1 md:grid-cols-[80px_1fr_auto] gap-6 md:gap-10 items-start py-8 border-t border-border-dim hover:bg-[var(--surface)]/50 transition-colors px-4 -mx-4">
            <div className="font-mono text-[11px] uppercase tracking-[0.25em] text-[var(--muted-foreground)] pt-1">PRJ_{p.id}</div>
            <div className="max-w-2xl">
              <h3 className="font-display font-semibold text-2xl md:text-[1.75rem] leading-tight tracking-tight text-foreground group-hover:text-[var(--accent)] transition-colors mb-3">{p.title}</h3>
              <p className="text-foreground/65 leading-relaxed text-[15px] mb-4">{p.description}</p>
              <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--muted-foreground)]">{p.label}</span>
            </div>
            <div className="font-mono text-[11px] uppercase tracking-[0.25em] text-[var(--accent)] opacity-60 group-hover:opacity-100 group-hover:translate-x-1 transition-all pt-1">VER REPO →</div>
          </motion.a>
        ))}
        <div className="grid grid-cols-1 md:grid-cols-[80px_1fr_auto] gap-6 md:gap-10 items-start py-8 border-t border-dashed border-[var(--accent)]/30 px-4 -mx-4 progress-shimmer">
          <div className="font-mono text-[11px] uppercase tracking-[0.25em] text-[var(--accent)] pt-1">PRJ_03</div>
          <div className="max-w-2xl opacity-60">
            <h3 className="font-display font-semibold text-2xl md:text-[1.75rem] leading-tight tracking-tight text-foreground/70 mb-3">Próximo proyecto en construcción</h3>
            <p className="text-foreground/55 leading-relaxed text-[15px] mb-4">Más detalles cuando esté listo para publicar.</p>
            <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--muted-foreground)]">NEXT_PROJECT.WIP</span>
          </div>
          <div className="pt-1"><Badge variant="warning" dot>EN PROCESO</Badge></div>
        </div>
      </div>
    </Section>
  );
        ...
        {viewMode === "carousel" && (
          <div className="flex items-center gap-4">
            <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--muted-foreground)] hidden sm:inline-block">
              CASO <span className="text-foreground font-bold">{String(selectedIndex + 1).padStart(2, "0")}</span> / {String(INVESTIGATIONS.length).padStart(2, "0")}
            </span>
                ...
                >
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {i.categories.map((c) => (
                        <span key={c} className="font-mono text-[9px] uppercase tracking-[0.2em] px-2 py-0.5 border border-border-dim text-foreground/70">
                          {c}
                        </span>
                      ))}
                    </div>
              ...
              <div className="flex flex-col justify-between h-full p-6 bg-[var(--surface)]/50 border border-dashed border-[var(--accent)]/30 opacity-75">
                <div>
                  ...
                  <p className="text-xs md:text-sm text-foreground/50 leading-relaxed">
                    Investigación forense en desarrollo. El análisis y la documentación de artefactos estarán disponibles próximamente.
                  </p>
                </div>
                <div className="mt-6 pt-4 border-t border-border-dim flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.25em]">
        ...
        <>
          ...
          <div className="hidden md:block border border-border-dim">
            <div className="grid grid-cols-[64px_1.4fr_1fr_140px_120px_60px] gap-4 px-5 py-3 border-b border-border-dim font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--muted-foreground)]">
              <div>N°</div><div>CASO</div><div>CATEGORÍA</div><div>PLATAFORMA</div><div>ESTADO</div><div></div>
            </div>
            {INVESTIGATIONS.map((i, idx) => (
              <motion.a key={i.id} href={i.href} target="_blank" rel="noreferrer" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.05 }} transition={{ duration: 0.4, delay: (idx % 4) * 0.05, ease: [0.16, 1, 0.3, 1] }} className="group grid grid-cols-[64px_1.4fr_1fr_140px_120px_60px] gap-4 items-center px-5 py-4 border-b border-border-dim last:border-b-0 transition-colors hover:bg-[color-mix(in_oklab,var(--accent)_4%,transparent)] hover:border-l-2 hover:border-l-[var(--accent)] hover:pl-[18px]" title={i.summary}>
                <div className="font-mono text-[11px] tracking-[0.2em] text-[var(--muted-foreground)] tabular-nums">{i.id.replace("LAB_", "")}</div>
                <div className="font-display font-semibold text-foreground text-[15px] tracking-tight group-hover:text-[var(--accent)] transition-colors">{i.title}</div>
                <div className="flex flex-wrap gap-1.5">{i.categories.map((c) => (<span key={c} className="font-mono text-[9px] uppercase tracking-[0.2em] px-1.5 py-0.5 border border-border-dim text-foreground/70">{c}</span>))}</div>
              ...
              <div className="md:text-right">
                {c.score ? (
                  <>
                    ...
                    {c.href && (<span className="mt-3 inline-block font-mono text-[10px] uppercase tracking-[0.25em] text-[var(--accent)] opacity-70 group-hover:opacity-100 transition-opacity">VER CERTIFICADO →</span>)}
                  </>
                ) : (
          ...
          return (
            ...
            >
              ...
              <div className="flex-1">
                <h4 className={`font-display font-semibold text-[14px] leading-snug tracking-tight transition-colors ${hasCert ? "text-foreground group-hover:text-[var(--accent)]" : "text-foreground/60"}`}>{c.title}</h4>
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-border-dim">
                ...
                {hasCert && (<span className="font-mono text-[9px] uppercase tracking-[0.2em] text-[var(--accent)] opacity-0 group-hover:opacity-100 transition-opacity">VER CERT →</span>)}
              </div>
            </motion.div>
          );
        })}
